...
def log_event(db, call: Call | None, event_type: str, status: str, message: str | None = None, integration_id: int | None = None):
    ...
    with SessionLocal() as db:
        ...
        try:
            ...
            if auto_transcribe:
                ...
                log_event(db, call, "transcription_queued", "success" if queued else "failed", None if queued else "Queue backend is unavailable", integration_id=integration_id)
                if not queued:
...
def run_worker() -> None:
    ...
    while running:
        ...
        try:
            ...
            print(f"invalid recording job payload: {payload}: {safe_error(exc)}")
