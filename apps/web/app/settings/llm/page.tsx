...
export default function Page() {
  ...
  const applyPresetDefaults = (presetId: string, preserveName = true) => {
    ...
    setForm((f: any) => ({
      ...
      name: preserveName && f.name ? f.name : p.label,
    }));
  };
  ...
  const load = async () => {
    ...
    setForm((f: any) => {
      ...
      const preset = nextPresets.find((p: Preset) => p.id === f.preset) || nextPresets.find((p: Preset) => p.id === "ollama") || nextPresets[0];
      if (!preset) return f;
  ...
  const testProvider = async (candidate: any) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setMessage({ kind: "success", text: `Test passed in ${data.latency_ms}ms using model ${data.model}.` });
      } else {
        const detail = data.provider_error ? JSON.stringify(data.provider_error).slice(0, 1000) : "Unknown error";
  ...
  return <main className="grid" style={{ gap: 16 }}>
    <section className="card">
      ...
      <form onSubmit={save} className="grid" style={{ gap: 12 }}>
        <div className="grid grid-2">
          <label>Preset
            <select value={form.preset} onChange={(e) => applyPresetDefaults(e.target.value)}>
          ...
          <label>Model
            ...
            <small>Use exact provider model ID, e.g. gpt-5.4-nano, qwen/qwen3-32b, gemini-3.5-flash.</small>
          </label>
          <label>API key
            <input type="password" value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder="Enter to set/update" />
            <small>Saved API keys are never shown. Leave empty to keep existing key.</small>
          </label>
          <label>Timeout seconds
            <input type="number" value={form.timeout_seconds} onChange={(e) => setForm({ ...form, timeout_seconds: Number(e.target.value) })} min={10} />
          </label>
        </div>
        {selectedPreset?.id === "openai" && <p style={{ margin: 0, color: "#4b5563" }}>Use exact API model IDs like gpt-5.4-nano, gpt-5.4-mini, or gpt-5.5. Do not use display names like 'GPT-5 nano'.</p>}
        ...
        <div className="actions">
          ...
          <button className="button button-secondary" type="button" onClick={() => testProvider(form)} disabled={isTesting}>{isTesting ? "Testing..." : "Test provider"}</button>
        </div>
      </form>
    ...
    <section className="card">
      ...
      <div className="grid" style={{ gap: 10 }}>
        {providers.map((p) => <article key={p.id} className="segment">
          <div className="actions" style={{ justifyContent: "space-between" }}>
            <strong>{p.name}</strong>
            <div className="actions">
              {p.is_active && <span className="badge badge-transcribed">active</span>}
              {p.api_key_configured && <span className="badge badge-uploaded">api_key_configured</span>}
            </div>
          </div>
          <p style={{ marginBottom: 6 }}>{p.preset} • {p.model}</p><p style={{ marginTop: 0, color: "#4b5563" }}>{p.base_url}</p>
          <div className="actions"><button className="button button-secondary" onClick={() => setForm({ ...p, api_key: "" })}>Edit</button><button className="button button-secondary" onClick={() => fetch(`${API_BASE_URL}/settings/llm/providers/${p.id}/activate`, { method: "POST" }).then(load)}>Activate</button><button className="button button-secondary" onClick={() => testProvider(p)} disabled={isTesting}>{isTesting ? "Testing..." : "Test"}</button></div>
        </article>)}
      </div>
    </section>
  </main>;
