...
export default function Page() {
  ...
  useEffect(() => { load().catch(() => setMessage({ kind: "error", text: t("settings.loadSttFailed") })); }, []);
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!res.ok) {
      ...
      return setMessage({ kind: "error", text: data?.detail || t("settings.sttProviderSaveFailed") });
    }
    setMessage({ kind: "success", text: form.id ? t("settings.sttProviderUpdated") : t("settings.sttProviderSaved") });
  ...
  const testProvider = async (candidate: any) => {
    ...
    try {
      ...
      if (data.ok) setMessage({ kind: "success", text: `OK: ${data.model || candidate.model || "provider"}. ${data.note || ""}` });
      else setMessage({ kind: "error", text: data.provider_error || t("settings.sttTestFailed") });
    } finally {
      setIsTesting(false);
    }
  };
  ...
  return (
    <AdminOnly><main className="grid" style={{ gap: 16 }}>
      ...
      <section className="card">
        ...
        {editingProvider && <p className="message" style={{ background: "#eff6ff", color: "#1d4ed8" }}>{t("settings.editingSttProvider")}: {editingProvider.name}</p>}
        ...
        <form ref={formRef} onSubmit={save} className="grid" style={{ gap: 12 }}>
          <div className="grid" style={{ gap: 10 }}>
            <label>{t("settings.preset")}</label><select value={form.preset} onChange={(event) => applyPresetDefaults(event.target.value)}>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.label}</option>)}</select>
            {selectedPreset?.note && <small>{selectedPreset.note}</small>}
            <label>{t("settings.providerName")}</label><input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required />
            ...
            {showBaseUrl && <><label>{t("settings.baseUrl")}</label><input value={form.base_url} onChange={(event) => setForm({ ...form, base_url: event.target.value })} placeholder="https://api.example.com/v1" /></>}
            ...
            <small>{t("settings.sttModelHelp")}</small>
            <label>{t("settings.apiKey")}</label><input type="password" value={form.api_key} onChange={(event) => setForm({ ...form, api_key: event.target.value })} />
            <small>{t("settings.apiKeyHelp")}</small>
            <label>{t("settings.timeoutSeconds")}</label><input type="number" value={form.timeout_seconds} onChange={(event) => setForm({ ...form, timeout_seconds: Number(event.target.value) })} min={10} />
          </div>
          <label><input type="checkbox" checked={form.is_active} onChange={(event) => setForm({ ...form, is_active: event.target.checked })} /> {t("settings.setActiveProvider")}</label>
      ...
      <section className="card"><h3>{t("settings.savedSttProviders")}</h3><div className="grid" style={{ gap: 10 }}>{providers.length === 0 ? <p>{t("settings.noSttProviders")}</p> : providers.map((provider) => <article key={provider.id} className="segment"><div className="actions" style={{ justifyContent: "space-between" }}><strong>{provider.name}</strong><div className="actions">{provider.is_active && <span className="badge badge-transcribed">{t("common.active")}</span>}{provider.api_key_configured && <span className="badge badge-uploaded">{t("settings.apiKeyConfigured")}</span>}</div></div><p style={{ marginBottom: 6 }}>{provider.provider_type} • {provider.model || "-"}</p><p style={{ marginTop: 0, color: "#4b5563" }}>{provider.base_url || t("common.local")}</p><div className="actions"><button className="button button-secondary" onClick={() => { setForm({ ...provider, api_key: "" }); setTimeout(() => formRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}>{t("common.edit")}</button><button className="button button-secondary" onClick={() => fetch(`${API_BASE_URL}/settings/stt/providers/${provider.id}/activate`, { method: "POST" }).then(load)}>{t("settings.activate")}</button><button className="button button-secondary" onClick={() => testProvider(provider)} disabled={isTesting}>{isTesting ? t("calls.working") : t("settings.testProvider")}</button></div></article>)}</div></section>
    </main></AdminOnly>
  );
}
